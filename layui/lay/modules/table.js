...
layui.define(['laytpl', 'laypage', 'layer', 'form', 'util'], function (exports) {
    ...
    Class.prototype.pullData = function (curr) {
        ...
        if (options.url) { //Ajax请求
            ...
            if (!options.page) {
                console.log("options param:", data)
            } else {
                //添加分页参数
                var params = {pageNum: curr, pageSize: options.limit}
                if (data.page) {
                    data.page = $.extend(data.page, params);
                } else {
                    data.page = params
                }
            }
            ...
            // if (tokenName) {
			// 	var ssoToken =localStorage.getItem(tokenName)
			// 	data._openx_head ={}
			// 	if(ssoToken){
			// 		try {
			// 			data._openx_head=JSON.parse(ssoToken)
			// 		}catch(exception){
